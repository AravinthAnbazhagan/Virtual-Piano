/*
let arr=['https://www.flipsidexr.com/files/docs/graphics/button_w-record.png','https://png.pngtree.com/element_our/png/20181205/stop-vector-icon-png_256694.jpg'];
var srcs=[];*/
...
function handleButtonClick(bId) {
    ...
    if (document.getElementById('recimg').src===arr[1]) {
        let s = audio.src;
        srcs.push(s);
        //alert(srcs);
    }
...
var blobUrl;
function changePic(){
    let s=document.getElementById('recimg').src;
    if (s===arr[0]) {
        document.getElementById('recimg').src = arr[1];
    } else{
        document.getElementById('recimg').src = arr[0];
        let proms = srcs.map(uri => fetch(uri).then(r => r.blob()));
        Promise.all(proms).then(blobs => {
            //alert("Enters prom");
            let blob = new Blob([...blobs]);
            blobUrl = URL.createObjectURL(blob);
        });
        //alert("Done with blobs");
    }
}
...
function down(){
    ...
    downloadLink.download = document.getElementById('fname').value+".mp3";
    downloadLink.click();
